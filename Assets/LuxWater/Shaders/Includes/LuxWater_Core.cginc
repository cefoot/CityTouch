...
//  Foam
    ...
    half    _SlopeAwareFoam;
    half    _SlopeToMask;
    half    _FoamFinalErosion;
#endif
...
//  Water Projectors
    ...
    UNITY_DECLARE_SCREENSPACE_TEXTURE(_LuxWater_FoamOverlay);
...
// Lighting helper functions
    ...
    half3 LuxWorldNormal(half3 tbn_0, half3 tbn_1, half3 tbn_2, half3 tnormal) {
        half3 worldN;
        worldN.x = dot(tbn_0, tnormal);
        worldN.y = dot(tbn_1, tnormal);
        worldN.z = dot(tbn_2, tnormal);
        return worldN;
    }
...
//  Vertex shader
    ...
    //  Slope aware Foam
        #if defined(GEOM_TYPE_BRANCH_DETAIL) && defined(EFFECT_BUMP)
            half foamAtSlope = (1 - smoothstep(0, 1, saturate(v.normal.y))) * _SlopeAwareFoam;
            v.color.g = foamAtSlope;       
        #endif

    //  Calculate wpos up front as we need it anyway and it allows us to optimize other calculations
    //  Tessellator will return wpos in v.vertex already!
        #if defined (LUXWATERUSESTESS)
    ...
    //  In case we use water projector and gerstnerwaves we need the undistorted screenUVs
    ...
    //  Handle flipped UV scrolling
        if (_UVDirection == 1) {
            _FinalBumpSpeed01.xy = _FinalBumpSpeed01.yx;
            _FinalBumpSpeed01.zw = _FinalBumpSpeed01.wz;
            _FinalBumpSpeed23.xy = _FinalBumpSpeed23.yx;
        }
    ...
    //  Gerstner Displacement
        ...
        //  In case we use UVs we derive the direction of the 4th wave from the tangent.
            #if defined(EFFECT_BUMP)
                _GDirectionCD.zw = v.tangent.zx; //(normalize(UnityObjectToWorldDir(v.tangent.xyz))).xz; // * v.tangent.w;
_GDirectionCD.zw = cross(v.tangent.xyz, v.normal).xz; // * v.tangent.w;
            #endif

            float3 vtxForAni = worldPos.xzz;                    //    + _WaterWorldShift.xyy;
            half3 nrml;
            float3 offsets;
            Gerstner (
            ...
            //  Combine and normalize Gerstner normals - shitty hacky...
                nrml = normalize(nrml * _GerstnerSecondaryWaves.x * 2 + tnormal);
            
            }

        //  In case we use uvs we do not assume that water is a flat plane so we have to properly tweak the normals
            #if defined(EFFECT_BUMP)
            //  https://www.gamedev.net/forums/topic/678043-how-to-blend-world-space-normals/
                float3 s = float3(0, 1, 0);
                float3 u = normalize(v.normal); //normalize(nrml);
                float3 t = normalize(nrml); //normalize(tnormal);
    ...
    fixed4 projectedNormal = tex2Dlod(_LuxWater_NormalOverlay, float4(projectionUVs, 0, 0));
            ...
            //  we can't use v.normal here due to the mask shader which would have to do the same, ouch!
                worldPos.y += (projectedNormal.b) * _LuxWater_Extrusion;
            }
        #endif
    ...
    //  In case we use world projection we must NOT rotate the tangent.
        ...
        //  Here we keep the old and wrong cross product to not break all metarials
...
//  Fragment shader
    ...
    //  if they are marked as underwater by the underwatermask.
            ...
            half4 underwatermask = UNITY_SAMPLE_SCREENSPACE_TEXTURE(_UnderWaterMask, i.grabUV.xy/i.grabUV.w);
    ...
    //  Normal Projectors
            ...
            fixed4 projectedNormal = UNITY_SAMPLE_SCREENSPACE_TEXTURE(_LuxWater_NormalOverlay, projectionUVs);
    ...
    //  Final normal
        tangentNormal.z *= facing;
    //  TODO: Check normals and bitangent...
...
float test = 1;
    ...
    //  Foam
        ...
        //  Slope based Foam (only if we use UV mapping)
            #if defined(EFFECT_BUMP)
                half foamAtSlope = i.color.g; // (1 - smoothstep(0, 1, (saturate(worldNormalFace.y)) )) * _SlopeAwareFoam;
                FoamSample = max(FoamSample, foamAtSlope);
                MaskBySlope = lerp(1, 0, _SlopeToMask * foamAtSlope);
            #endif
        ...
        //  Mask foam by the water's normals
            ...
            FoamMaskFromNormal = (1.0h - FoamMaskFromNormal * _FoamNormalMaskScale
            #if defined(EFFECT_BUMP)
                * MaskBySlope
            #endif
            );
        ...
        //  Add Foam Projectors
            #if defined (USINGWATERPROJECTORS)
                fixed4 projectedFoam = UNITY_SAMPLE_SCREENSPACE_TEXTURE(_LuxWater_FoamOverlay, projectionUVs);
        ...
        //  Deep Foam
            ...
            FoamSample = saturate(smoothstep( FoamSample, /*saturate*/(FoamSample + _FoamFinalErosion) , rawFoamSample.a) );
            ...
            half3 FoamNormal = rawFoamSample.rgb * 2 - 1;
            FoamNormal.xy *= Foam.a * _FoamNormalScale;
    ...
    //  Set missing data for BRDF
            ...
            half3 Refraction = UNITY_SAMPLE_SCREENSPACE_TEXTURE(_GrabTexture, snappedDistortedGrabUVs);
        #else
            half3 Refraction = 0;
        #endif
...
//  -----------------------------------------------------------------------
//  Aura: add fog to refraction when underwater
        #if defined(FOG_AURA)
            #if defined (UNITY_PASS_FORWARDBASE)
                if (backside) {
                    Aura2_ApplyFog(Refraction, float3(i.grabUV.xy/i.grabUV.w, sceneDepth)); // i.frustumSpacePosition);
                }
            #endif
        #endif
...
//  Taken from URP Lighting
float3 halfDir = Unity_SafeNormalize (lightDir + worldViewDir);
...
//  Taken from URP Lighting to prevent highlight from going nuts.
#if defined (SHADER_API_MOBILE) || defined (SHADER_API_SWITCH)
    #define LW_HALF_MIN 6.103515625e-5  // 2^-14, the same value for 10, 11 and 16-bit:
...
// diffuseLighting += lightScattering * Foam.rgb * _FoamTranslucencyIntensity;
    ...
    //  Custom fog
        ...
        #elif defined(FOG_AURA)
            #if defined (UNITY_PASS_FORWARDBASE)
                if (!backside) {
                    Aura2_ApplyFog(c.rgb, float3(i.grabUV.xy/i.grabUV.w, surfaceEyeDepth));
                }
            #endif
        #endif
